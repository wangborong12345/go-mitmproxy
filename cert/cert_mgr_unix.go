...
func NewSystemTrustCertMgr() *UnixSystemTrustCertMgr {
	return &UnixSystemTrustCertMgr{validCaSystemRelated: getValidCaSystemRelated()}
}
...
func (mgr *UnixSystemTrustCertMgr) Install(ca *x509.Certificate) error {
	...
	path := filepath.Join(related.Path, CaCertCerFile)
...
func (mgr *UnixSystemTrustCertMgr) Uninstall(cert *x509.Certificate) error {
	related := mgr.validCaSystemRelated
	path := fmt.Sprintf("%s%s", related.Path, CaCertCerFile)
